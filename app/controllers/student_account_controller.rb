class StudentAccountController < ApplicationController
  ...
  def activity_and_performances
    ...
    if classroom_activity_pairing
      ...
      if classroom_student_users
        ...
        performances = StudentPerformance.joins("left join activity_levels on student_performances.activity_level_id = activity_levels.id")
          .where({classroom_activity_pairing_id: classroom_activity_pairing.id, student_user_id: @current_student_user.id})
          .order("activity_levels.name ASC, student_performances.created_at ASC")
          .as_json
  ...
  def save_activity_goal
    ...
    if !activity_goal 
      activity_goal = ActivityGoal.new(params.require(:activity_goal).permit(:score_goal, :goal_date, :classroom_activity_pairing_id, :activity_level_id, :notes))
    else
      activity_goal.update_attributes({score_goal: params[:activity_goal][:score_goal], goal_date: params[:activity_goal][:goal_date], score_goal: params[:activity_goal][:score_goal], activity_level_id: params[:activity_goal][:activity_level_id], notes: params[:activity_goal][:notes]})
    end
